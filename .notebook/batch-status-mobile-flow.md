...
- `frontend/mini_mart_management_mobile_app/lib/screens/batch_status_screen.dart` derives the visible status from the active flag, expiry day, and remaining quantity. Its expiry-monitoring UI uses the existing batch API to show 7-day critical and 30-day warning counts, a search/product filter, and batch cards sorted by the nearest expiry.
